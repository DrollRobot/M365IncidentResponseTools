...
```
Get-NonInteractiveLog [[-UserObject] <PSObject[]>] [-Days <Int32>] [-Beta <Boolean>] [-Xml <Boolean>]
 [-Script <Boolean>] [-Open <Boolean>] [-ProgressAction <ActionPreference>] [<CommonParameters>]
```

## DESCRIPTION
A convenience wrapper around Get-SignInLog that sets -NonInteractive automatically.
...
Date range and output behavior are identical to Get-SignInLog.
...
### EXAMPLE 2
```
Get-NonInteractiveLog -UserObject $User -Days 30
