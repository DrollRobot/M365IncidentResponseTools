function Add-IpInfoToSheet {
    ...
    # Apply conditional formatting rules (color-coding by provider type) for each enriched column.
    foreach ($Name in $ColMap.Keys) {
        Add-IpAddressConditionalFormatting -Worksheet $Worksheet -ColumnName $Name
    }
}
