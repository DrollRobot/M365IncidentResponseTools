function Find-PowerShellCommand {
    ...
    .EXAMPLE
        Find-PowerShellCommand -Path .\Connect-IRTGraph.ps1

        Returns the unique command names invoked in the specified file.

    .EXAMPLE
        Find-PowerShellCommand -Text 'Get-MgUser | Where-Object Enabled'
    ...
    if ($PSCmdlet.ParameterSetName -eq 'Path') {
        Write-Trace "Find-PowerShellCommand: parsing file '$Path'"
        $ast = [System.Management.Automation.Language.Parser]::ParseFile(
            $Path, [ref] $tokens, [ref] $parseErrors)
    }
    else {
        Write-Trace "Find-PowerShellCommand: parsing text input"
        $ast = [System.Management.Automation.Language.Parser]::ParseInput(
            $Text, [ref] $tokens, [ref] $parseErrors)
    }

    if ($parseErrors) {
        Write-Trace "Find-PowerShellCommand: $($parseErrors.Count) parse error(s)"
    }
    ...
    $names = foreach ($command in $commandAsts) {
        ...
        if ($name) {
            Write-Trace "Find-PowerShellCommand: found '$name'"
            $name
        }
        else {
            Write-Trace "Find-PowerShellCommand: skipped dynamic call (no static name)"
        }
    }
