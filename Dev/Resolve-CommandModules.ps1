...
function Find-CommandModule {
    ...
    .EXAMPLE
        Find-CommandModule -Name Get-MgUser

        Resolves a single command and reports its module, path, and source.

    .EXAMPLE
        Find-PowerShellCommand -Path .\Connect-IRTGraph.ps1 | Find-CommandModule
    ...
    process {
        foreach ($commandName in $Name) {
            ...
            if (-not $command) {
                Write-Trace "Find-CommandModule: not found, skipping '$commandName'"
                [PSCustomObject]@{
                    Name       = $commandName
                    Module     = $null
                    ModulePath = $null
            ...
            while ($command.CommandType -eq 'Alias') {
                Write-Trace ("Find-CommandModule: '$commandName' is an alias for " +
            ...
            Write-Trace "Find-CommandModule: '$commandName' -> $($module.Name) [$source]"
            [PSCustomObject]@{
                Name       = $commandName
                Module     = $module.Name
