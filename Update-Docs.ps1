...
$DocsPath = Join-Path $PSScriptRoot 'docs\commands'
...
Import-Module (Join-Path $PSScriptRoot 'M365IncidentResponseTools.psd1') -Force
...
(Get-Module M365IncidentResponseTools).ExportedFunctions.Keys |
    Where-Object { -not (Test-Path (Join-Path $DocsPath "$_.md")) } |
